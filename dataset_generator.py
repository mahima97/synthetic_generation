...
def randomColor(img):
	color_range = np.arange(0.5,1.5,0.25)
	im = img.convert('RGB')
	r, g, b = im.split()
	ratio = random.choice(color_range)

	# r = r.point(lambda i: i * ratio)

	# g = g.point(lambda i: i * ratio)

	# b = b.point(lambda i: i * ratio)

	color_array = [r,g,b]

	# Recombine back to RGB image
	rgb_img = Image.merge('RGB', tuple(random.sample(color_array, 3)))
		...
		for idx, obj in enumerate(all_objects):
			...
			for i in range(len(blending_list)):
				if blending_list[i] == 'none' or blending_list[i] == 'motion':
					foreground = randomColor(foreground)
...
def generate_synthetic_dataset(args):
	...
	print(syn_img_files,anno_files)
...
def parse_args():
	...
	parser.add_argument("--num",
	  help="Number of times each image will be in dataset", default=100, type=int)
